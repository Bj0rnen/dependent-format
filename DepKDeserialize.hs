...
class DepKDeserialize (f :: ks) where
    type SerConstraints f (xs :: LoT ks) :: Constraint
    ...
    depKSerialize :: SerConstraints f xs => TheseK f xs -> [Word8]
    depKDeserialize
    ...
    default depKSerialize :: forall xs. (GenericK' f xs, DepKDeserializeK (RepK f), SerConstraintsK (RepK f) xs) => TheseK f xs -> [Word8]
    depKSerialize (TheseK (Proxy :: Proxy xs) a) =
        depKSerializeK ((withDict (interpretVarsIsJustVars @xs) (fromK @_ @f @xs a \\ genericKInstance @f @xs)) :: RepK f xs)
...
class (DepKDeserialize a, SerConstraints a 'LoT0, Require a 'AtomNil 'DZ) => Serialize a
instance (DepKDeserialize a, SerConstraints a 'LoT0, Require a 'AtomNil 'DZ) => Serialize a
serialize :: forall a. Serialize a => a -> [Word8]
serialize a = depKSerialize (TheseK (Proxy @'LoT0) a)
...
depKSerialize1Up
    :: forall k ks (f :: k -> ks) (x :: k) (xs :: LoT ks)
    . ( DepKDeserialize f
      , SerConstraints1Up (f x) xs ~ SerConstraints1Up (f x) xs
      , SerConstraints1Up (f x) xs
      )
    => TheseK (f x) xs -> [Word8]
depKSerialize1Up (TheseK (Proxy :: Proxy xs) a) = depKSerialize @_ @f (TheseK (Proxy :: Proxy (x :&&: xs)) a)
...
instance DepKDeserialize Int8 where
    ...
    depKSerialize (TheseK _ a) = [fromIntegral a]
    depKDeserialize _ = AnyK Proxy <$> withoutKnowledge do
        (bs :: [Word8]) <- get
        case bs of
            [] -> throwError $ DeserializeError "No more bytes to read!"
...
instance (SingKind k, Serialize (Demote k)) => DepKDeserialize (Sing :: k -> Type) where
    ...
    depKSerialize (TheseK _ a) = serialize @(Demote k) (FromSing a)
    depKDeserialize
...
class DepKDeserializeK (f :: LoT ks -> Type) where
    ...
    depKSerializeK :: SerConstraintsK f xs => f xs -> [Word8]
    depKDeserializeK
        :: forall d (ds :: DepStateList d) (as :: AtomList d ks)
        .  RequireK f as ds
...
instance (DepKDeserialize (AtomKonConstructor t)) => DepKDeserializeK (Field t) where
    ...
    depKSerializeK (Field a :: Field t xs) = depKSerialize @(AtomKonKind t) @(AtomKonConstructor t) (TheseK (Proxy @(InterpretAll (AtomKonAtomList t) xs)) (unsafeCoerce a))
    depKDeserializeK
...
instance (DepKDeserializeK f, DepKDeserializeK g) => DepKDeserializeK (f :*: g) where
    ...
    depKSerializeK (a :*: b) = depKSerializeK a ++ depKSerializeK b
    depKDeserializeK p =
...
instance DepKDeserializeK f => DepKDeserializeK (M1 i c f) where
    ...
    depKSerializeK (M1 a) = depKSerializeK a
    depKDeserializeK p =
        depKDeserializeK @_ @f p >>>= \(AnyKK a) ->
        ireturn $ AnyKK (M1 a)
...
instance DepKDeserializeK f => DepKDeserializeK (Exists k (f :: LoT (k -> ks) -> Type)) where
    ...
    depKSerializeK (Exists (a :: f (x ':&&: xs))) =
        case unsafeCoerce (Dict @(SerConstraintsK (Exists k f) xs)) :: Dict (SerConstraintsK f (x ':&&: xs)) of
            Dict ->
                depKSerializeK a
    depKDeserializeK
        :: forall d (ds :: DepStateList d) (as :: AtomList d ks)
